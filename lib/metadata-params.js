...
function getSelectedRecords(selectedRecords, rows) {
    const previouslySelectedIds = [];
    if (selectedRecords && selectedRecords.length > 0) 
        rows.forEach(row => previouslySelectedIds[row.Id] = selectedRecords.find(selectedRecord => selectedRecord.Id == row.Id));

    return previouslySelectedIds;
}

const buildSelectorRowForType = function(record, selectedRecordsForType) {
    return selectorRowByType[this.type](record);
}
...
export { metadataTypes, queryStringByType, selectorColumnHeadersByType, buildSelectorRowForType, buildReportRowForType, reportColumnHeadersByType, getSelectedRecords };