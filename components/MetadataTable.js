...
function MetadataTable({columns, rows, selector, setParentSelectedRows, previouslySelectedIds, type}) {
    ...
    React.useEffect(() => {
        console.log('effecting previously selected ids ', previouslySelectedIds);
    ...
    return (
        <table className={`${styles.parent} mt-5 w-100`}>
            <thead>
                <tr>
                    {showSelector && <th ><input ref={selectAllCheckboxRef} onChange={() => selectAll()} type="checkbox" value={true} id={`checkBoxAll_${type}`} className="mx-auto"/><label for={`checkBoxAll_${type}`} /></th>}
                    {columns.map(header => <th key={header}>{header}</th>)}
                </tr>
            </thead>
            <tbody>
                {rows.map((row, index) => <DataRow key={index} type={type} rowIndex={index} updateSelection={updateSelectedRow} showSelector={showSelector} {...row}/>)}
            </tbody>
        </table>
    );
}
...
function DataRow({updateSelection, defaultSelected, selected, cells, rowIndex, showSelector, type}) {
    ...
    return (
        <tr className="py-3">     
            {showSelector && <td><input onChange={() => updateSelection(rowIndex)} checked={selected} value={selected} type="checkbox" id={`checkBox_${rowIndex}_${type}`}/><label for={`checkBox_${rowIndex}_${type}`} /></td>}
